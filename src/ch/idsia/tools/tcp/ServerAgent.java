...
public class ServerAgent extends BasicAIAgent implements Agent
{
    ...
    private void createServer(int port) {
        this.server = new Server(port, Environment.numberOfObservationElements, Environment.numberOfButtons+1);
...
//        System.out.println("tmpData size = " + tmpData.length());
    ...
    private boolean[] receiveAction() throws IOException, NullPointerException
    {
        ...
        if (data == null || data.startsWith("reset"))
            return null;
        boolean[] ret = new boolean[Environment.numberOfButtons+1];
//        String s = "[";
        for (int i = 0; i < Environment.numberOfButtons+1; ++i)
        {
